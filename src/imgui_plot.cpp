...
inline void AddTextVertical(ImDrawList* DrawList, const char* text, ImVec2 pos, ImU32 text_color) {
  ...
  char c;
  // XXX ImGuiContext& g = *GImGui;
  // XXX ImVec2 text_size = CalcTextSize(text);
  while ((c = *text++)) {
...
inline void GetTicks(float tMin, float tMax, int nMajor, int nMinor, std::vector<Tick>& out) {
  ...
  // XXX const int nfrac = std::max((int)-floor(log10(interval)), 0);
...
inline void TransformTicks(std::vector<Tick>& ticks, float tMin, float tMax, float pMin, float pMax) {
  ...
  for (auto& tk : ticks)
    tk.pixels =
        (pMin + m * ((float)tk.plot - tMin));  // IM_ROUND used previoiusly, but causes distortion in moving plot
}

/* XXX
inline void
RenderPlotItemLineAA(const PlotItem& item, const PlotInterface& plot, const ImRect& pix, ImDrawList& DrawList) {
  if (item.data.size() < 2)
    return;
...
inline void
RenderPlotItemLine(const PlotItem& item, const PlotInterface& plot, const ImRect& pix, ImDrawList& DrawList) {
  if (item.data.size() < 2)
    return;
...
inline void
RenderPlotItemScatter(const PlotItem& item, const PlotInterface& plot, const ImRect& pix, ImDrawList& DrawList) {
  const ImU32 col = GetColorU32(item.color);
  const float mx = (pix.Max.x - pix.Min.x) / (plot.x_axis.maximum - plot.x_axis.minimum);
  const float my = (pix.Max.y - pix.Min.y) / (plot.y_axis.maximum - plot.y_axis.minimum);
  for (size_t i = 0; i < item.data.size(); ++i) {
...
inline void
RenderPlotItemXBar(const PlotItem& item, const PlotInterface& plot, const ImRect& pix, ImDrawList& DrawList) {
  const ImU32 col = GetColorU32(item.color);
  const float mx = (pix.Max.x - pix.Min.x) / (plot.x_axis.maximum - plot.x_axis.minimum);
  const float my = (pix.Max.y - pix.Min.y) / (plot.y_axis.maximum - plot.y_axis.minimum);
...
inline void
RenderPlotItemYBar(const PlotItem& item, const PlotInterface& plot, const ImRect& pix, ImDrawList& DrawList) {
  const ImU32 col = GetColorU32(item.color);
  const float mx = (pix.Max.x - pix.Min.x) / (plot.x_axis.maximum - plot.x_axis.minimum);
  const float my = (pix.Max.y - pix.Min.y) / (plot.y_axis.maximum - plot.y_axis.minimum);
  const float halfSize = 0.5f * item.size;
...
void Plot(const char* label_id, PlotInterface* plot_ptr, PlotItem* items, int nItems, const ImVec2& size) {
  ...
  const ImU32 color_bg =
      plot.background_color.w == -1 ? GetColorU32(ImGuiCol_WindowBg) : GetColorU32(plot.background_color);
  const ImU32 color_border =
      plot.border_color.w == -1 ? GetColorU32(ImGuiCol_Text, 0.5f) : GetColorU32(plot.border_color);

  const ImVec4 color_xAxis =
      plot.x_axis.color.w == -1 ? ImGui::GetStyle().Colors[ImGuiCol_Text] * ImVec4(1, 1, 1, 0.25f) : plot.x_axis.color;
  ...
  const ImVec4 color_yAxis =
      plot.y_axis.color.w == -1 ? ImGui::GetStyle().Colors[ImGuiCol_Text] * ImVec4(1, 1, 1, 0.25f) : plot.y_axis.color;
  ...
  const bool renderX = (plot.x_axis.show_grid || plot.x_axis.show_tick_marks || plot.x_axis.show_tick_labels) &&
                       plot.x_axis.divisions > 1;
  const bool renderY = (plot.y_axis.show_grid || plot.y_axis.show_tick_marks || plot.y_axis.show_tick_labels) &&
  ...
  if (plot.show_legend && nItems > 0) {
    ...
    legend_content_bb =
        ImRect(grid_bb.Min + legend_offset, grid_bb.Min + legend_offset + ImVec2(maxLabelWidth, nItems * textHeight));
  ...
  if (plot._dragging_x || plot._dragging_y) {
    ...
    if (!xLocked && plot._dragging_x) {
      ...
      float delX =
          dir * IO.MouseDelta.x * (plot.x_axis.maximum - plot.x_axis.minimum) / (grid_bb.Max.x - grid_bb.Min.x);
    ...
    if (!yLocked && plot._dragging_y) {
      ...
      float delY =
          dir * IO.MouseDelta.y * (plot.y_axis.maximum - plot.y_axis.minimum) / (grid_bb.Max.y - grid_bb.Min.y);
  ...
  const ImRect pix(
      plot.x_axis.flip ? grid_bb.Max.x : grid_bb.Min.x,
  ...
  if (plot.x_axis.show_tick_marks) {
    for (auto& xt : xTicks)
      DrawList.AddLine(
          {xt.pixels, grid_bb.Max.y}, {xt.pixels, grid_bb.Max.y - (xt.major ? 10.0f : 5.0f)}, color_border, 1);
  }
  if (plot.y_axis.show_tick_marks) {
    for (auto& yt : yTicks)
      DrawList.AddLine(
          {grid_bb.Min.x, yt.pixels}, {grid_bb.Min.x + (yt.major ? 10.0f : 5.0f), yt.pixels}, color_border, 1);
  }

  // render crosshairs
  if (plot.show_crosshairs && grid_hovered && frame_hovered && !(plot._dragging_x || plot._dragging_y) &&
  ...
  if (plot.show_legend && nItems > 0) {
    ...
    for (int i = 0; i < nItems; ++i) {
      ...
      DrawList.AddText(
          ...
          items[i].show ? color_txt : color_txt_dis,
          items[i].label.c_str());
    }
  }
  ...
  if (plot.title != "") {
    ...
    DrawList.AddText(
        ImVec2(canvas_bb.GetCenter().x - title_size.x * 0.5f, canvas_bb.Min.y), color_txt, plot.title.c_str());
  }
  ...
  if (plot.y_axis.show_tick_labels) {
    ...
    for (auto& yt : yTicks)
      DrawList.AddText(
          {grid_bb.Min.x - textOffset - yt.size.x, yt.pixels - 0.5f * yt.size.y}, color_ytxt, yt.txt.c_str());
    DrawList.PopClipRect();
  }
  ...
  if (ImGui::BeginPopup("##Context")) {
    ...
    ImGui::DragFloat(
        ...
        &plot.x_axis.maximum,
        0.01f + 0.01f * (plot.x_axis.maximum - plot.x_axis.minimum),
        plot.x_axis.minimum + FLT_EPSILON,
        INFINITY);
    if (plot.x_axis.lock_max) {
      ImGui::PopItemFlag();
      ImGui::PopStyleVar();
    ...
    ImGui::DragFloat(
        ...
        &plot.y_axis.maximum,
        0.01f + 0.01f * (plot.y_axis.maximum - plot.y_axis.minimum),
        plot.y_axis.minimum + FLT_EPSILON,
        INFINITY);
    if (plot.y_axis.lock_max) {
      ImGui::PopItemFlag();
      ImGui::PopStyleVar();
